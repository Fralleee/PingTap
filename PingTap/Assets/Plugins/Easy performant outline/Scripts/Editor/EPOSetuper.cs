...
namespace EPOOutline
{
  ...
  public class EPOSetuper : EditorWindow
  {
    ...
    {
      System.Collections.Generic.HashSet<RenderPipelineAsset> activeAssets = PipelineAssetUtility.ActiveAssets;

      foreach (RenderPipelineAsset asset in activeAssets)
      {
    ...
    public void OnGUI()
    {
      ...
      if (!CheckHasActiveRenderers())
      {
        ...
        {
          string path = EditorUtility.SaveFilePanelInProject("Asset location", "Rendering asset", "asset", "Select the folder to save rendering asset");
          if (string.IsNullOrEmpty(path))
            return;

          string pathNoExt = Path.ChangeExtension(path, string.Empty);
          pathNoExt = pathNoExt.Substring(0, pathNoExt.Length - 1);

          UnityEngine.Rendering.Universal.ForwardRendererData rendererAsset = PipelineAssetUtility.CreateRenderData();
          RenderPipelineAsset asset = PipelineAssetUtility.CreateAsset(rendererAsset);
      ...
      {
        System.Collections.Generic.HashSet<RenderPipelineAsset> assets = PipelineAssetUtility.ActiveAssets;
        foreach (RenderPipelineAsset asset in assets)
        {
          ...
          string text = string.Format("There is no outline feature added to the pipeline asset called '{0}'. Click select to find asset that is missing outline features", asset.name);
