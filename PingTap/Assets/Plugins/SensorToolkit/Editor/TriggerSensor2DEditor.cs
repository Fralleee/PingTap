...
namespace SensorToolkit
{
  ...
  public class TriggerSensor2DEditor : Editor
  {
    ...
    {
      List<GameObject> detected = triggerSensor.DetectedObjects;
      List<GameObject> undetected = triggerSensor.ObjectVisibilities.Keys.Where(go => !detected.Contains(go)).ToList();
      ...
      foreach (GameObject go in detected)
      {
        ...
        bool debug = triggerSensor.ShowRayCastDebug.Contains(go);
    ...
    {
      bool hasRB = triggerSensor.GetComponent<Rigidbody2D>() != null;
      if (hasRB)
      {
