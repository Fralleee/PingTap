...
namespace SensorToolkit
{
  ...
  public class RangeSensorEditor : Editor
  {
    ...
    {
      List<GameObject> detected = rangeSensor.DetectedObjects;
      List<GameObject> undetected = rangeSensor.ObjectVisibilities.Keys.Where(go => !detected.Contains(go)).ToList();
      ...
      foreach (GameObject go in detected)
      {
        ...
        bool debug = rangeSensor.ShowRayCastDebug.Contains(go);
    ...
    void startTesting()
    {
      if (isTesting || Application.isPlaying || rangeSensor == null)
        return;
