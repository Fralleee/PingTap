...
namespace SensorToolkit
{
  ...
  public class TriggerSensor : BaseVolumeSensor
  {
    ...
    void FixedUpdate()
    {
      ...
      foreach (KeyValuePair<Collider, int> test in isColliderStale)
      {
        tempColliderList.Add(test.Key);
      }
      foreach (Collider c in tempColliderList)
      {
    ...
    {
      if (!isColliderStale.ContainsKey(other))
      { addCollider(other); }
    ...
    {
      GameObject newDetected = base.addCollider(other);
    ...
    new void removeCollider(Collider other)
    {
      ...
      GameObject detectionLost = base.removeCollider(other);
    ...
    void sensorDetectionEvents()
    {
      ...
      List<GameObject>.Enumerator detectedEnumerator = tempGOList.GetEnumerator();
      ...
      HashSet<GameObject>.Enumerator previousDetectedEnumerator = previousDetectedObjects.GetEnumerator();
      while (previousDetectedEnumerator.MoveNext())
      {
        GameObject go = previousDetectedEnumerator.Current;
    ...
    {
      bool hasRB = GetComponent<Rigidbody>() != null;
      if (hasRB)
      {
