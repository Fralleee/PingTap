  ...
  public abstract class BaseVolumeSensor : Sensor
  {
    ...
    public override List<GameObject> DetectedObjects
    {
      get
      {
        ...
        {
          Dictionary<GameObject, float>.Enumerator objectVisibilityEnumerator = objectVisibility.GetEnumerator();
          while (objectVisibilityEnumerator.MoveNext())
          {
            KeyValuePair<GameObject, float> current = objectVisibilityEnumerator.Current;
            GameObject go = current.Key;
        ...
        {
          Dictionary<GameObject, List<Collider>>.Enumerator rigidBodyCollidersEnumerator = rigidBodyColliders.GetEnumerator();
          while (rigidBodyCollidersEnumerator.MoveNext())
          {
            GameObject go = rigidBodyCollidersEnumerator.Current.Key;
        ...
        {
          Dictionary<GameObject, List<Collider>>.Enumerator gameObjectCollidersEnumerator = gameObjectColliders.GetEnumerator();
          while (gameObjectCollidersEnumerator.MoveNext())
          {
            GameObject go = gameObjectCollidersEnumerator.Current.Key;
            if (go != null && go.activeInHierarchy && !shouldIgnore(go))
            {
              detectedObjects.Add(go);
            }
    ...
    public override float GetVisibility(GameObject go)
    {
      ...
      else if (objectVisibility.ContainsKey(go))
    ...
    class RayCastTargets
    {
      ...
      {
        List<Transform> visibleList = new List<Transform>();
        for (int i = 0; i < isTargetVisible.Count; i++)
        {
          if (isTargetVisible[i])
            visibleList.Add(targetTransforms[i]);
        }
        return visibleList;
      }
      ...
      public List<Vector3> GetVisibleTargetPositions()
      {
        ...
        if (targetTransforms != null)
        {
          ...
          {
            if (isTargetVisible[i])
              visibleList.Add(targetTransforms[i].position);
          }
        }
        else
        {
          ...
          {
            if (isTargetVisible[i])
              visibleList.Add(go.transform.TransformPoint(targetPoints[i]));
          }
        }
        return visibleList;
      }
    ...
    protected GameObject addCollider(Collider c)
    {
      ...
      GameObject newDetection = DetectionMode == SensorMode.Colliders ? newColliderDetection : newRigidBodyDetection;
      ...
      else if (RequiresLineOfSight && newDetection != null)
      {
        ...
        RayCastTargets targets = getRayCastTargets(newDetection);
        if (TestLOSTargetsOnly && !targets.IsTransforms())
          return null;
    ...
    protected GameObject removeCollider(Collider c)
    {
      ...
      GameObject detectionLost = DetectionMode == SensorMode.Colliders ? colliderDetectionLost : rigidBodyDetectionLost;
    ...
    {
      gameObjectList.Clear();
      Dictionary<GameObject, List<Collider>>.Enumerator collidersGameObjectsEnumerator = gameObjectColliders.GetEnumerator();
      while (collidersGameObjectsEnumerator.MoveNext())
      {
        GameObject go = collidersGameObjectsEnumerator.Current.Key;
        if (go == null)
      ...
      gameObjectList.Clear();
      Dictionary<GameObject, List<Collider>>.Enumerator rigidBodyGameObjectsEnumerator = rigidBodyColliders.GetEnumerator();
      while (rigidBodyGameObjectsEnumerator.MoveNext())
      {
        GameObject go = rigidBodyGameObjectsEnumerator.Current.Key;
        if (go == null)
        { gameObjectList.Add(go); }
      }
      for (int i = 0; i < gameObjectList.Count; i++)
      {
        List<Collider> colliderList = rigidBodyColliders[gameObjectList[i]];
    ...
    {
      Dictionary<GameObject, List<Collider>>.Enumerator collidersEnumerator = gameObjectColliders.GetEnumerator();
      while (collidersEnumerator.MoveNext())
      {
        List<Collider> colliderList = collidersEnumerator.Current.Value;
    ...
    protected void refreshLineOfSight()
    {
      ...
      {
        Dictionary<GameObject, List<Collider>>.Enumerator gosEnumerator = rigidBodyColliders.GetEnumerator();
        while (gosEnumerator.MoveNext())
        {
          GameObject go = gosEnumerator.Current.Key;
          if (go == null || shouldIgnore(go))
            continue;
          RayCastTargets targets = getRayCastTargets(go);
          if (TestLOSTargetsOnly && !targets.IsTransforms())
            continue;
      ...
      {
        Dictionary<GameObject, List<Collider>>.Enumerator gosEnumerator = gameObjectColliders.GetEnumerator();
        while (gosEnumerator.MoveNext())
        {
          GameObject go = gosEnumerator.Current.Key;
          if (go == null || shouldIgnore(go))
    ...
    float testObjectVisibility(GameObject go, RayCastTargets targets)
    {
      ...
      {
        Vector3 testPoint = testPoints[i];
    ...
    RayCastTargets getRayCastTargets(GameObject go)
    {
      ...
      {
        LOSTargets losTargets = go.GetComponent<LOSTargets>();
    ...
    List<Vector3> generateRayCastTargets(GameObject go)
    {
      ...
      if (DetectionMode == SensorMode.Colliders)
        cs = gameObjectColliders[go];
      else
        cs = rigidBodyColliders[go];
    ...
    bool isInLineOfSight(GameObject go, Vector3 testPoint, out Vector3 obstructionPoint)
    {
      ...
      Vector3 toGoCentre = testPoint - transform.position;

      Ray ray = new Ray(transform.position, toGoCentre.normalized);
      RaycastHit hitInfo;
    ...
    Vector3 getRandomPointInColliders(GameObject goRoot, List<Collider> colliders)
    {
      ...
      for (int i = 0; i < colliders.Count; i++)
      {
        Collider c = colliders[i];
        rc = c;
        float v = c.bounds.size.x * c.bounds.size.y * c.bounds.size.z;
        r -= v / totalVolume;
        if (r <= 0f)
          break;
      }
      ...
      Vector3 rp = new Vector3(Random.Range(-.5f, .5f), Random.Range(-.5f, .5f), Random.Range(-.5f, .5f));
    ...
    {
      Dictionary<GameObject, RayCastTargets>.Enumerator rayCastTargetsEnumerator = rayCastTargets.GetEnumerator();
      while (rayCastTargetsEnumerator.MoveNext())
      {
        RayCastTargets rayCastTarget = rayCastTargetsEnumerator.Current.Value;
