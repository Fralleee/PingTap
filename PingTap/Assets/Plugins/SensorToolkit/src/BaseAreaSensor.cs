  ...
  public abstract class BaseAreaSensor : Sensor
  {
    ...
    public override List<GameObject> DetectedObjects
    {
      get
      {
        ...
        {
          Dictionary<GameObject, List<Collider2D>>.Enumerator rigidBodyCollidersEnumerator = rigidBodyColliders.GetEnumerator();
        ...
        {
          Dictionary<GameObject, List<Collider2D>>.Enumerator gameObjectCollidersEnumerator = gameObjectColliders.GetEnumerator();
    ...
    class RayCastTargets
    {
      ...
      {
        List<Vector2> visibleList = new List<Vector2>();
    ...
    {
      gameObjectList.Clear();
      Dictionary<GameObject, List<Collider2D>>.Enumerator collidersGameObjectsEnumerator = gameObjectColliders.GetEnumerator();
      ...
      gameObjectList.Clear();
      Dictionary<GameObject, List<Collider2D>>.Enumerator rigidBodyGameObjectsEnumerator = rigidBodyColliders.GetEnumerator();
      ...
      for (int i = 0; i < gameObjectList.Count; i++)
      {
        List<Collider2D> colliderList = rigidBodyColliders[gameObjectList[i]];
    ...
    {
      Dictionary<GameObject, List<Collider2D>>.Enumerator collidersEnumerator = gameObjectColliders.GetEnumerator();
      while (collidersEnumerator.MoveNext())
      {
        List<Collider2D> colliderList = collidersEnumerator.Current.Value;
    ...
    protected void refreshLineOfSight()
    {
      ...
      {
        Dictionary<GameObject, List<Collider2D>>.Enumerator gosEnumerator = rigidBodyColliders.GetEnumerator();
      ...
      {
        Dictionary<GameObject, List<Collider2D>>.Enumerator gosEnumerator = gameObjectColliders.GetEnumerator();
        while (gosEnumerator.MoveNext())
        {
          GameObject go = gosEnumerator.Current.Key;
          if (go == null || shouldIgnore(go))
    ...
    bool isInLineOfSight(GameObject go, Vector2 testPoint, out Vector2 obstructionPoint)
    {
      ...
      Vector2 toGoCentre = testPoint - (Vector2)transform.position;
    ...
    Vector2 getRandomPointInColliders(GameObject goRoot, List<Collider2D> colliders)
    {
      ...
      for (int i = 0; i < colliders.Count; i++)
      {
        Collider2D c = colliders[i];
        rc = c;
        float v = c.bounds.size.x * c.bounds.size.y;
        r -= v / totalArea;
        if (r <= 0f)
          break;
      }
      ...
      Vector2 rp = new Vector2(Random.Range(-.5f, .5f), Random.Range(-.5f, .5f));
