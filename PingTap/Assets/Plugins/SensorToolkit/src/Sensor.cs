...
namespace SensorToolkit
{
  ...
  public abstract class Sensor : MonoBehaviour
  {
    ...
    {
      List<GameObject>.Enumerator detectedEnumerator = DetectedObjects.GetEnumerator();
    ...
    {
      List<GameObject>.Enumerator detectedEnumerator = DetectedObjectsOrderedByDistance.GetEnumerator();
    ...
    {
      List<GameObject>.Enumerator detectedEnumerator = DetectedObjectsOrderedByDistance.GetEnumerator();
      List<Component> filtered = new List<Component>();
      while (detectedEnumerator.MoveNext())
      {
        GameObject go = detectedEnumerator.Current;
    ...
    private GameObject nearestToPoint(List<GameObject> gos, Vector3 point)
    {
      ...
      List<GameObject>.Enumerator gosEnumerator = gos.GetEnumerator();
      while (gosEnumerator.MoveNext())
      {
        GameObject go = gosEnumerator.Current;
        float d = Vector3.SqrMagnitude(go.transform.position - point);
  ...
  public class DistanceFromPointComparer : IComparer<GameObject>
  {
    ...
    {
      float d1 = Vector3.SqrMagnitude(x.transform.position - Point);
      float d2 = Vector3.SqrMagnitude(y.transform.position - Point);
      if (d1 < d2)
      { return -1; }
      else if (d1 > d2)
      { return 1; }
