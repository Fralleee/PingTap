  ...
  public class SoldierAI : MonoBehaviour
  {
    ...
    List<GameObject> enemiesSpotted
    {
      get
      {
        ...
        {
          TeamMember detectedTeam = detected[i].GetComponent<TeamMember>();
    ...
    IEnumerator DefaultState()
    {
      ...
      List<Holdable> pickups = pickupsSpotted;
      if (pickups.Count > 0)
      {
        if (pickups[0].IsHeld && pickups[0].Holder.GetComponent<TeamMember>().Team != team)
        {
          StartCoroutine(AttackState(pickups[0].Holder));
          yield break;
        }
      ...
      { StartCoroutine(AttackState()); yield break; }

      float countdown = 1f;
      while (countdown > 0f)
      {
        // Make way to the center of the map
        Vector3 targetDirection = SteerSensor.GetSteeredDirection(-transform.position);
    ...
    IEnumerator AttackState(GameObject target = null)
    {
      if (target == null)
      {
        List<GameObject> enemies = enemiesSpotted;
        if (enemies.Count == 0)
      ...
      {
        // Strafe
        Vector3 strafeDirection = new Vector3(Random.Range(-1f, 1f), 0f, Random.Range(-1f, 1f)).normalized;
        while (cooldown > 0f)
        {
          if (target == null)
            break;
    ...
    IEnumerator CarryToBaseState()
    {
      ...
      Vector3 baseDirection = MyBase.transform.position - transform.position;
