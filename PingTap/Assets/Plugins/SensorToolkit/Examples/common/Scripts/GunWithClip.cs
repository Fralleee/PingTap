...
namespace SensorToolkit.Example
{
  public class GunWithClip : MonoBehaviour
  {
    ...
    public void Fire()
    {
      ...
      {
        GameObject bullet = Instantiate(Bullet, Nozzle.transform.position, Nozzle.transform.rotation);
        RaySensor raySensor = bullet.GetComponent<RaySensor>();
        raySensor.IgnoreList.Clear();
        raySensor.IgnoreList.Add(gameObject);
        GameObject effect = Instantiate(FireEffect, Nozzle.transform.position, Nozzle.transform.rotation);
