...
namespace SensorToolkit.Example
{
  public class BoxSpawner : MonoBehaviour
  {
    ...
    void spawn()
    {
      ...
      if (nextSlot == -1)
        return; // No spawn slots available
      ...
      GameObject newInst = Instantiate(ToSpawn, pos, transform.rotation);
      newInst.transform.SetParent(transform.parent);
      if (Team != Teams.None)
      {
        TeamMember instTeam = newInst.GetComponent<TeamMember>();
        if (instTeam != null)
        {
    ...
    {
      Vector3 dims = new Vector3(SizeX / 2f, SizeY / 2f, SizeZ / 2f);
      Vector3 randVector = new Vector3(Random.Range(-1f, 1f), Random.Range(-1f, 1f), Random.Range(-1f, 1f));
      Vector3 pos = Vector3.Scale(dims, randVector) + transform.position;
      return pos;
    }
