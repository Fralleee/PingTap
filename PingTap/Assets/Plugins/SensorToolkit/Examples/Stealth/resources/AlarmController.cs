...
namespace SensorToolkit.Example
{
  public class AlarmController : MonoBehaviour
  {
    ...
    {
      int nearestToFrom = getNearestWaypointIndex(from.transform.position);
      int nearestToTripper = getNearestWaypointIndex(whoTrippedAlarm.transform.position);
      int nWaypoints = Mathf.Abs(nearestToFrom - nearestToTripper);
      bool ascending = nearestToTripper > nearestToFrom;
      Transform[] path = new Transform[nWaypoints];
      for (int i = 0; i < nWaypoints; i++)
      {
        int offset = ascending ? i : -i;
    ...
    IEnumerator alarmRoutine()
    {
      ...
      while (true)
      {
        float intensity = (Mathf.Sin(FlashFrequency * Time.time * Mathf.PI * 2f) + 1f) / 2f * startIntensity;
    ...
    int getNearestWaypointIndex(Vector3 toPos)
    {
      ...
      {
        float dist = (toPos - LinearLevelWaypoints[i].position).sqrMagnitude;
