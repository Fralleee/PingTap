...
namespace SensorToolkit.Example
{
  ...
  public class GuardAI : MonoBehaviour
  {
    ...
    IEnumerator Chase()
    {
      ...
      Transform[] chasePath = AlarmController.Instance.PathToWhoTrippedAlarm(gameObject);
    ...
    {
      System.Collections.Generic.List<TeamMember> spottedCharacters = Sight.GetDetectedByComponent<TeamMember>();
    ...
    int getNearestWaypointIndex()
    {
      ...
      {
        float dist = (transform.position - PatrolPath[i].position).sqrMagnitude;
    ...
    {
      Vector3 rv = new Vector3(Random.Range(-1f, 1f), 0f, Random.Range(-1f, 1f)).normalized;
