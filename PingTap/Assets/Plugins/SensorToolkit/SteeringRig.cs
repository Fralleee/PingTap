...
namespace SensorToolkit
{
  ...
  public class SteeringRig : MonoBehaviour
  {
    ...
    void Update()
    {
      ...
      Vector3 targetMoveDirection = (Destination - RB.transform.position).normalized;
      Vector3 avoidanceMoveDirection = GetSteeredDirection(targetMoveDirection);

      // If we haven't been assigned a direction to face then turn to face the direction we will move in.
      if (!IsDirectionToFaceAssigned)
        FaceDirectionKinematic(avoidanceMoveDirection);
    ...
    void FixedUpdate()
    {
      ...
      Vector3 targetMoveDirection = Destination - RB.transform.position;
      Vector3 avoidanceMoveDirection = GetSteeredDirection(targetMoveDirection);

      // If we haven't been assigned a direction to face then turn to face the direction we will move in.
      if (!IsDirectionToFaceAssigned)
        FaceDirectionForces(avoidanceMoveDirection);
      ...
      if (YAxis)
      {
        float zRot = RB.rotation.eulerAngles.z;
        if (zRot > 180f)
          zRot -= 360f;
        float zForce = Mathf.Clamp(zRot / 10f, -1f, 1f) * TurnForce;
    ...
    void FaceDirectionForces(Vector3 direction)
    {
      ...
      {
        float a = signedAngleXZ(RB.transform.forward, direction);
      ...
      float torque = Mathf.Clamp(angle / 20f, -1f, 1f) * TurnForce;
    ...
    Vector3 accumForces(Vector3 targetDirection)
    {
      ...
      Vector3 f = previousAttractionVector + previousRepulsionVector;
    ...
    {
      Vector3 dest = Vector3.Scale(targetDirection, axisConstraint);
    ...
    Vector3 repulsionForce()
    {
      ...
      {
        RaySensor s = sensors[i];
        s.IgnoreList = IgnoreList;
        s.Pulse();
        if (!s.IsObstructed)
          continue;
        float obsRatio = Mathf.Pow(1f - (s.ObstructionRayHit.distance / s.Length), 1f / AvoidanceSensitivity); // 0 when unobstructed, 1 when touching
    ...
    {
      float aa = Mathf.Atan2(a.x, a.z) * Mathf.Rad2Deg;
      float ba = Mathf.Atan2(b.x, b.z) * Mathf.Rad2Deg;
    ...
    public void OnDrawGizmosSelected()
    {
      ...
      Vector3 attractionPoint = transform.position + previousAttractionVector * 2f;
      Vector3 repulsionPoint = attractionPoint + previousRepulsionVector * 2f;
      Vector3 avoidancePoint = transform.position + previousAvoidanceVector * 2f;
