  ...
  public class RaySensor2D : Sensor
  {
    ...
    public override List<GameObject> DetectedObjects
    {
      get
      {
        ...
        List<GameObject>.Enumerator detectedEnumerator = detectedObjectsInternal.GetEnumerator();
        while (detectedEnumerator.MoveNext())
        {
          GameObject go = detectedEnumerator.Current;
    ...
    void Update()
    {
      if (Application.isPlaying && SensorUpdateMode == UpdateMode.EachFrame)
        testRay();
    }
    ...
    void testRay()
    {
      bool canDetectMultiple = !layerMaskIsSubsetOf(DetectsOnLayers, ObstructedByLayers) && (IgnoreList == null || IgnoreList.Count == 0);
      clearDetectedObjects();
    ...
    void detectionEvents()
    {
      ...
      HashSet<GameObject>.Enumerator lostDetectionEnumerator = previousDetectedObjects.GetEnumerator();
    ...
    public void OnDrawGizmosSelected()
    {
      ...
      if (Radius > 0f)
      {
        ...
        Vector3 line = endPosition - transform.position;
        if (line == Vector3.zero)
        {
          line = Vector3.forward * Length;
        }
        Vector3 center = transform.position + line / 2f;
        float length = line.magnitude;
        Quaternion rotation = Quaternion.LookRotation(line.normalized, Vector3.back) * Quaternion.Euler(90f, 0f, 0f);
