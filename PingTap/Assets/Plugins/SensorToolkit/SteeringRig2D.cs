...
namespace SensorToolkit
{
  ...
  public class SteeringRig2D : MonoBehaviour
  {
    ...
    {
      float deltaAngle = signedAngleXY(RB.transform.up, direction);
      float maxDelta = (TurnSpeed * Time.deltaTime) * Mathf.Min(1f, Mathf.Abs(deltaAngle) / 20f);
    ...
    void FixedUpdate()
    {
      ...
      Vector2 targetMoveDirection = Destination - (Vector2)RB.transform.position;
      Vector2 avoidanceMoveDirection = GetSteeredDirection(targetMoveDirection);
    ...
    {
      float angle = signedAngleXY(RB.transform.up, direction);
    ...
    Vector3 accumForces(Vector3 targetDirection)
    {
      ...
      Vector2 f = previousAttractionVector + previousRepulsionVector;
    ...
    {
      float aa = Mathf.Atan2(a.x, a.y) * Mathf.Rad2Deg;
      float ba = Mathf.Atan2(b.x, b.y) * Mathf.Rad2Deg;
    ...
    public void OnDrawGizmosSelected()
    {
      ...
      Vector2 attractionPoint = (Vector2)transform.position + previousAttractionVector * 2f;
      Vector2 repulsionPoint = attractionPoint + previousRepulsionVector * 2f;
      Vector2 avoidancePoint = (Vector2)transform.position + previousAvoidanceVector * 2f;
